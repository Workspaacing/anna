...
pub struct LanguageSettingsContent {
    ...
    /// Anna's Prettier integration settings.
    ...
    /// when you type '(', Anna will automatically add a closing ')' at the correct position.
    ///
    /// Default: true
    pub use_autoclose: Option<bool>,
    /// Whether to automatically surround text with characters for you. For example,
    /// when you select text and type '(', Anna will automatically surround text with ().
    ///
    /// Default: true
...
#[serde(rename_all = "snake_case")]
pub enum Formatter {
    /// Format files using Anna's Prettier integration (if applicable),
    /// or falling back to formatting via language server.
    #[default]
    Auto,
    /// Do not format code.
    None,
    /// Format code using Anna's Prettier integration.
    Prettier,
...
pub struct LanguageTaskSettingsContent {
    ...
    pub enabled: Option<bool>,
    /// Use LSP tasks over Anna language extension ones.
    /// If no LSP tasks are returned due to error/timeout or regular execution,
    /// Anna language extension tasks will be used instead.
    ///
    /// Other Anna tasks will still be shown:
    /// * Anna task from either of the task config file
    /// * Anna task from history (e.g. one-off task was spawned before)
