...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Anna instance.
    ...
    /// Whether to optimize Anna's interface for assistive technology such as
    /// screen readers.
    ///
    /// Default: false
    pub accessible_mode: Option<bool>,
    /// Whether to use the system provided dialogs for Open and Save As.
    /// When set to false, Anna will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Anna will use the built-in prompts.
    ...
    /// Which fullscreen mode the `anna::ToggleFullScreen` action enters (macOS only).
    ///
    /// Default: native
    ...
    /// Controls whether Anna or the window manager or compositor draws window decorations on Linux.
    ///
    /// Default: client
...
/// Configures what draws Anna's window decorations on Linux.
...
pub enum WindowDecorations {
    /// Anna draws its own window decorations/titlebar (client-side decoration).
    #[default]
    Client,
...
pub enum CliDefaultOpenBehavior {
    /// Open directories as a new workspace in the current Anna window's sidebar.
    #[default]
...
pub enum DefaultOpenBehavior {
    /// Open projects in the current Anna window.
    #[default]
    #[strum(serialize = "Add to Existing Window")]
    ExistingWindow,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Anna.
    #[default]
    LastSession,
