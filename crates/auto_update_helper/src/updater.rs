...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("Anna.exe"), p("old\\Anna.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\anna.exe"), p("old\\bin\\anna.exe")),
        Job::move_file(p("bin\\anna"), p("old\\bin\\anna")),
        //
        ...
        // Copy new files
        Job::move_file(p("install\\Anna.exe"), p("Anna.exe")),
        Job::move_file(p("install\\bin\\anna.exe"), p("bin\\anna.exe")),
        Job::move_file(p("install\\bin\\anna"), p("bin\\anna")),
        //
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("Anna.exe"),
        app_dir.join("bin\\anna.exe"),
        app_dir.join("bin\\anna"),
...
fn zed_launch_command(app_dir: &Path, launch_arguments: &[OsString]) -> std::process::Command {
    let mut command = std::process::Command::new(app_dir.join("Anna.exe"));
...
pub(crate) fn perform_update(
    ...
    if launch {
        ...
        let _child = zed_launch_command(app_dir, launch_arguments)
            .spawn()
            .context("Failed to launch Anna after update")?;
    }
...
mod test {
    ...
    fn test_zed_launch_command_preserves_arguments() {
        let arguments = vec![
            ...
            OsString::from(r"C:\Anna Data"),
        ];
        let command = zed_launch_command(Path::new(r"C:\Program Files\Anna"), &arguments);

        assert_eq!(
            command.get_program(),
            Path::new(r"C:\Program Files\Anna\Anna.exe").as_os_str()
        );
        assert_eq!(
