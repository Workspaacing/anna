...
const LIGHT_THEMES: [&str; 2] = ["Anna Neutral Light", "One Light"];
const DARK_THEMES: [&str; 2] = ["Anna Neutral Dark", "One Dark"];
const FAMILY_NAMES: [SharedString; 2] = [
    SharedString::new_static("Anna"),
...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex().gap_2().child(Label::new("Base Keymap")).child(
        ToggleButtonGroup::two_rows(
            ...
            [
                ToggleButtonWithIcon::new("Anna", IconName::Anna, |_, _, cx| {
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "Anna can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some("Automatically mark all new projects as trusted to unlock all Anna's features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
    ...
    h_flex()
        ...
        .child(
            v_flex()
                ...
                .child(
                    Label::new("Open files and folders from the terminal with the anna command")
                        .color(Color::Muted),
                ),
        )
        .child(
