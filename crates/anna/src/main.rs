...
use crate::anna::{OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn main() {
    ...
    // `anna --askpass` Makes zed operate in nc/netcat mode for use with askpass
    ...
    // `anna --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    let failed_single_instance_check = if *anna_env_vars::ZED_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::anna::listen_for_cli_connections(open_listener.clone()).is_err()
        }

        #[cfg(target_os = "windows")]
        {
            !crate::anna::windows_only_instance::handle_single_instance(
                open_listener.clone(),
                &args,
            )
        }
    ...
    app.run(move |cx| {
        ...
        anna::watch_settings_files(fs.clone(), cx);
        ...
        anna::init(cx);
        #[cfg(target_os = "macos")]
        anna::move_to_applications::init(cx);
...
// from Anna's own project history. Oldest first so the newest ends up on top.
...
fn handle_open_request(request: OpenRequest, app_state: Arc<AppState>, cx: &mut App) {
    if let Some(kind) = request.kind {
        match kind {
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| {
                        window.dispatch_action(
                            Box::new(anna_actions::Extensions {
            ...
            OpenRequestKind::Setting { setting_path } => {
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(anna_actions::OpenSettings), cx),
                        Some(setting_path) => window.dispatch_action(
                            Box::new(anna_actions::OpenSettingsAt {
                                path: setting_path,
                                target: None,
                            }),
                            cx,
            ...
            OpenRequestKind::GitCommit { sha } => {
                let base_open_options = anna::open_options_for_request(
    ...
    if let Some(connection_options) = request.remote_connection {
        ...
        let base_open_options = anna::open_options_for_request(open_behavior, &location, cx);
        cx.spawn(async move |cx| {
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Anna would exit silently.
...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Anna`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/anna`.
    /// On Windows, the default is `%LOCALAPPDATA%\Anna`.
    ...
    /// Instructs anna to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Anna from starting, so you can't run `anna: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Run anna in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// The PID of the Anna process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    ...
    /// Unix socket path for IPC with the parent Anna process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
