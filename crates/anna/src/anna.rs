...
use anna_actions::{
    About, OpenAnnaUrl, OpenBrowser, OpenDocs, OpenProjectTasks, OpenServerSettings,
...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &anna_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&anna_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &anna_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenAnnaUrl, _, cx| {
        ...
        .register_action(|workspace, action: &anna_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &anna_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &anna_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &anna_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &anna_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &anna_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &anna_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &anna_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterAnnaScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_anna_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterAnnaScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterAnnaScheme>(),
                            format!(
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &anna_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
                        ...
                        MessageNotification::new(format!("Invalid user settings file\n{error}"), cx)
                            ...
                            .primary_on_click(|window, cx| {
                                window.dispatch_action(
                                    anna_actions::OpenSettingsFile.boxed_clone(),
                                    cx,
                                );
    ...
    show_app_notification(notification_id, cx, move |cx| {
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_icon(IconName::Settings)
                .primary_on_click(|window, cx| {
                    window.dispatch_action(anna_actions::OpenKeymapFile.boxed_clone(), cx);
                    cx.emit(DismissEvent);
                })
        })
    });
...
fn open_worktree_setup_tasks_file(
    workspace: &mut Workspace,
    _: &anna_actions::OpenWorktreeSetupTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &anna_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    if let Some(task) = open_local_file(
...
mod tests {
    ...
    async fn test_open_remote_from_existing_connection_reuses_window(
        ...
        window
            .update(cx, |multi_workspace, window, cx| {
                ...
                window.dispatch_action(
                    Box::new(anna_actions::OpenRemote {
    ...
    async fn test_open_paths_action(cx: &mut TestAppContext) {
        ...
        cx.run_until_parked();
        let multi_workspace_2 = cx
            .read(|cx| cx.windows()[1].downcast::<MultiWorkspace>())
            .unwrap();
        multi_workspace_2
    ...
    async fn test_open_file_in_many_spaces(cx: &mut TestAppContext) {
        ...
        // Opening an unrelated directory with default options creates a new window.
        cx.update(|cx| {
            open_paths(
        ...
        .await
        .unwrap();
        assert_eq!(cx.update(|cx| cx.windows().len()), 4);
    }
