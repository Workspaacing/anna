...
use anna_actions::ExtensionCategoryFilter;
...
actions!(
    ...
    [
        ...
        #[action(deprecated_aliases = ["wu::InstallDevExtension"])]
        InstallDevExtension,
    ]
);
...
#[action(namespace = anna, deprecated_aliases = ["wu::RebuildDevExtension"])]
...
pub fn init(cx: &mut App) {
    ...
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        workspace
            .register_action(
                move |workspace, action: &anna_actions::Extensions, window, cx| {
    ...
    fn on_extension_installed(
            ...
            workspace
                .update(cx, |_workspace, cx| {
                    window.dispatch_action(
                        anna_actions::theme_selector::Toggle {
        ...
        if !icon_themes.is_empty() {
            workspace
                .update(cx, |_workspace, cx| {
                    window.dispatch_action(
                        anna_actions::icon_theme_selector::Toggle {
        ...
        for feature in &self.upsells {
            let banner = match feature {
                Feature::ExtensionBasedpyright => self.render_feature_upsell_banner(
                    "Basedpyright (Python language server) support is built-in to Anna!".into(),
                    "https://zed.dev/docs/languages/python#basedpyright".into(),
                ),
                Feature::ExtensionRuff => self.render_feature_upsell_banner(
                    "Ruff (linter for Python) support is built-in to Anna!".into(),
