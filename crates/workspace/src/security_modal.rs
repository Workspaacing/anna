...
impl ModalView for SecurityModal {
    ...
    fn on_before_dismiss(&mut self, _: &mut Window, _: &mut Context<Self>) -> DismissDecision {
        match self.trusted {
            Some(false) => DismissDecision::Dismiss(true),
            Some(true) => DismissDecision::Dismiss(true),
                ...
                v_flex()
                    ...
                    .child(
                        v_flex()
                            ...
                            .child(
                                Label::new(
                                    "Review .anna/settings.json for any extensions or commands configured by this project.",
                                )
                                .color(Color::Muted),
                            ),
                    )
