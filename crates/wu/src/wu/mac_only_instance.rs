...
fn address() -> SocketAddr {
    ...
    // Anna uses a different port range than Zed so both apps can run side by side
...
fn instance_handshake() -> String {
    let handshake = match *release_channel::RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Anna Editor Dev Instance Running",
        ReleaseChannel::Stable => "Anna Editor Stable Instance Running",
    };
