# Wu -- the fork delta vs. upstream Zed

Repo: `C:/Users/USER/Documents/wu-main`. Fork owner / repo: **`Workspaacing/wu`**.
Upstream base pin: `C:/Users/USER/Documents/wu-main/UPSTREAM_VERSION` = `01acd0ee8e906dd0ec8b526fe08da94444a5e2af` (a zed-industries/zed commit).
App version: `crates/wu/Cargo.toml` = `1.0.6`. Release channel file: `crates/wu/RELEASE_CHANNEL` = `dev`.

---

## 1. What Wu is and is NOT

### Is
- A **fork of Zed** that keeps the editor core, GPU renderer (GPUI), LSP/DAP/tree-sitter tooling, git UI, terminal, tasks, extensions, and remote (SSH/WSL) editing.
- Tagline (`README.md:4`, `docs/index.html`): "The fast, native code editor that doesn't get in your way." Name from *wu wei*.
- Public docs: https://wu.farshed.me (`docs/CNAME`). Source: https://github.com/Workspaacing/wu
...
Corollary from the docs page: *"If a setting or key binding from the Zed docs belongs to one of these features, it has no effect in Wu."*

### Wu-specific additions (not in upstream Zed)
...
- `script/upstream-sync` + `.github/workflows/upstream-sync.yml` -- an automated, history-free upstream merge pipeline (see section 7).

---

## 2. Naming and branding rules

### Product / user-facing (always "Wu")
- `crates/paths/src/paths.rs:19` -- `pub const APP_NAME: &str = "Wu";` plus a const-evaluated `APP_NAME_LOWERCASE = "wu"`. **This single const drives every platform data/config/cache/state dir.** Its doc comment says explicitly: *"Forks should change this to avoid colliding with Zed's user data."*
- Bundle identifiers: **`me.farshed.Wu`** (stable) and **`me.farshed.Wu-Dev`** (dev):
  ...
  - `crates/release_channel/src/lib.rs:46-48` (Windows `app_identifier()` = `Wu-Editor-Dev` / `Wu-Editor-Stable`)
...
- Display names: `ReleaseChannel::display_name()` = `"Wu"` / `"Wu Dev"`. macOS root menu is `"Wu"` (`crates/wu/src/wu/app_menus.rs:60`).
- HTTP User-Agent: `format!("Wu/{} ({}; {})", ...)` -- `crates/wu/src/main.rs:485-490`.
- Binaries: `wu` (`crates/wu/Cargo.toml`, `[[bin]] name = "wu"`), `wu_visual_test_runner`. The CLI crate builds a bin named `cli` but presents itself as `wu` (`crates/cli/src/main.rs:51`).
- Release assets (`crates/auto_update/src/auto_update.rs` `github_asset_name`, `.github/workflows/release.yml`): `Wu-<arch>.dmg`, `wu-linux-<arch>.tar.gz`, `Wu-x86_64.exe`, `wu-remote-server-<os>-<arch>.gz`.

### Directories and files
| Thing | Value | Where |
|---|---|---|
| Project-local config dir | `.wu/` (legacy fallback `.zed/`) | `crates/paths/src/paths.rs:501-593` |
| Project settings | `.wu/settings.json` | `paths.rs:534` |
| Project tasks | `.wu/tasks.json` | `paths.rs:549` |
| Project debug scenarios | `.wu/debug.json` | `paths.rs:580` |
| Config dir | `~/.config/wu`, `%APPDATA%\Wu` | `paths.rs:149-168` |
| Data dir | `~/Library/Application Support/Wu`, `$XDG_DATA_HOME/wu`, `%LOCALAPPDATA%\Wu` | `paths.rs:170-194` |
| Remote server dir on host | `.wu_server` / `.wu_wsl_server` | `paths.rs:69-81` |
| URL scheme | **`wu://`** | `crates/wu/src/wu/open_listener.rs:142-165`, `crates/cli/src/main.rs:34`, `osx_url_schemes = ["wu"]` |

`paths.rs` implements a **`.wu` preferred / `.zed` fallback** rule: the `.zed` file is used only when it exists and the `.wu` one does not. Preserve that when touching config resolution.

### Action namespaces
- `crates/wu_actions/src/lib.rs` is the app-level action crate. App actions use `#[action(namespace = wu)]`, giving user-visible IDs like `wu::OpenSettings`, `wu::About`, `wu::OpenDocs`, `wu::Extensions`, `wu::IncreaseBufferFontSize`.
- Deprecated aliases are namespaced `wu_actions::...` (e.g. `#[action(deprecated_aliases = ["wu_actions::OpenSettings"])]`), i.e. already rewritten from Zed's `zed_actions::`.
...
1. **Upstream attribution and URLs**: `README.md`, `docs/*`, `script/upstream-sync`, `.github/workflows/upstream-sync.yml`, GitHub issue links in code comments.
2. **Zed services Wu genuinely still uses**: `api.zed.dev` for the extension registry (section 4), `https://zed.dev/docs` for documentation deep-links.
3. **Crate names kept from upstream**: `zlog`, `zlog_settings`, `ztracing`, `ztracing_macro`. Also `crates/proto/proto/zed.proto`, `crates/client/src/zed_urls.rs`, `crates/grammars/src/zed-keybind-context`, `script/licenses/zed-licenses.toml`.
4. **Packaging file names not renamed** (contents are Wu-branded): `crates/wu/resources/windows/zed.iss`, `crates/wu/resources/windows/zed.sh`, `crates/wu/resources/zed.desktop.in`, `crates/wu/resources/zed.entitlements`, `crates/wu/resources/flatpak/zed.metainfo.xml.in`.
...
- `crates/client/src/client.rs:693` -- `pub const ZED_URL_SCHEME: &str = "wu";` (name says zed, value is wu).
- `crates/client/src/zed_urls.rs` -- module doc still describes Zed; exposes `terms_of_service()` and `ai_privacy_and_security()`, which are **dead concepts in Wu**.
...
- `crates/cli/src/main.rs` -- CLI still accepts a `--zed <path>` flag (`args.zed`); the flatpak path resolves to a binary named `wu-editor`.
- `crates/proto/Cargo.toml` description: "Shared protocol for communication between the Zed app and the zed.dev server" -- inaccurate for Wu.
...
| Crate | Status in Wu | What it is actually used for |
|---|---|---|
...
| `crates/remote`, `crates/remote_connection`, `crates/remote_server` | **Live and supported** | SSH and WSL remote editing. `remote_server` is a separate daemon binary shipped as `wu-remote-server-<os>-<arch>.gz`; transports in `crates/remote/src/transport/{ssh,wsl}.rs`. |
...
| `crates/zlog`, `zlog_settings`, `ztracing`, `ztracing_macro` | Live, upstream-named | Logging (`zlog::init()`, `zlog::init_output_file`) and tracing/Tracy (`ztracing::init()`, `--features tracy`). |

---

## 4. Networking -- does Wu phone home?
...
1. **Auto-update -> GitHub (Wu's own repo).** `crates/auto_update/src/auto_update.rs:233`
   `const GITHUB_RELEASES_API_URL: &str = "https://api.github.com/repos/Workspaacing/wu/releases";`
   ...
   - Release-notes / commit links: `https://github.com/Workspaacing/wu/releases/tag/v{version}` and `https://github.com/Workspaacing/wu/commits/main/` (`auto_update.rs:415,417`).
   - Asset digests are SHA-256 verified when GitHub publishes one (`ReleaseAsset.digest`, `sha2` dependency).
   - The `wu-remote-server-*` binary is fetched through the same mechanism (`github_asset_name`).
2. **Extension registry -> `api.zed.dev`.** `crates/extension_host/src/extension_host.rs:787,951,996` call `http_client.build_zed_api_url(...)`, and `crates/http_client/src/http_client.rs:276-289` maps base `https://zed.dev` to `https://api.zed.dev`. Endpoints: `/extensions...`, `/extensions/{id}/download`, `/extensions/{id}/{version}/download`. This is **deliberate** -- `docs/docs/index.html`: *"Extensions come from the Zed extension registry, and every Zed extension works in Wu."*
   ...
   - `build_zed_cloud_url`, `build_zed_cloud_url_with_query`, and `build_zed_llm_url` exist in `http_client` but **have no callers in Wu**.
...
5. **Doc links** -- `crates/wu/src/wu.rs:101` `DOCS_URL = "https://wu.farshed.me/docs"`, but `crates/release_channel/src/lib.rs:10` `ZED_DOCS_URL = "https://zed.dev/docs"` still backs `ReleaseChannel::docs_url(slug)` for deep links into Zed's reference docs (intentional; Wu's docs defer to Zed's).

**Net: no telemetry, no crash upload, no account, no panic upload (`install_panic_hook` writes only to `paths::logs_dir()/panics.log`). Two first-party hosts: `api.github.com/repos/Workspaacing/wu` for updates and `api.zed.dev` for extensions.**
...
`README.md:70`: *"Wu is licensed under GPL-3.0-or-later with Apache-2.0 components where marked."* Wu is a derivative work of Zed and carries the same split.
...
- Copying from **upstream Zed** is fine and expected (same licenses, same crate names). Match the source crate's license to the destination crate's license.
...
Entry point: **`crates/wu/src/main.rs`** (`[[bin]] name = "wu", path = "src/main.rs"`; `default-members = ["crates/wu"]` in the root `Cargo.toml`).
...
- **New global action or app-level command** -> declare in `crates/wu_actions/src/lib.rs` (`#[action(namespace = wu)]` or `actions!(wu, [...])`), then handle it in `wu::init` (`crates/wu/src/wu.rs:171-286`) for app scope, or in `register_actions` (`crates/wu/src/wu.rs:680`) for workspace scope.
...
- **New `wu://` URL verb** -> `crates/wu/src/wu/open_listener.rs:142-165`.
...
3. **`git blame` / `git diff` against upstream is unavailable.** The working copy is not a git repo, and by design (`script/upstream-sync`) *no Zed history ever enters this repository* -- upstream changes land as single squashed commits produced by `git merge-tree`.
4. **`script/upstream-sync` encodes the fork policy.** Read it before changing file layout:
   - `DROPPED_PREFIXES` -- upstream edits under `.github/`, `docs/`, `extensions/`, `tooling/`, `ci/`, `nix/`, `assets/prompts/`, `assets/sounds/`, `legal/`, `.agents/`, `.zed/`, `CONTRIBUTING.md`, etc. are discarded.
   - **Whole crates that do not exist in HEAD are dropped automatically.** This is the mechanism that keeps agent/collab/vim from creeping back in on every sync. Creating a stub crate with one of those names would silently re-open the door.
   - `REVIEW_PREFIXES = (crates/zed/)` -- upstream `crates/zed` maps onto `crates/wu`; files Wu rewrote fall through and need a human to port or discard.
   - `PINNED_FILES = (crates/wu/RELEASE_CHANNEL)`.
   - Ordering uses upstream's own `crates/zed/Cargo.toml` version, not commit dates.
5. **Grep for `\bzed\b`, not `zed`.** `Serialized` / `normalized` / `humanized` / `memoized` produce hundreds of false positives.
6. **`ZED_*` env vars are load-bearing.** `crates/wu/build.rs` emits `ZED_COMMIT_SHA` / `ZED_BUILD_ID`; `.github/workflows/release.yml` writes `crates/wu/RELEASE_CHANNEL`; `crates/release_channel/src/lib.rs` does `include_str!("../../wu/RELEASE_CHANNEL")` (with a `__do_not_set_zed_release_channel` cfg escape hatch for nix/crane vendoring). Renaming any of these breaks CI or the build.
7. **`base_keymap` serde value is `"Zed"`** even though the settings UI shows `"Wu"`. Changing the serde name breaks every existing user settings file.
8. **`server_url` default `https://zed.dev` is not dead config** -- it is the base for the extension registry via `build_zed_api_url`. Do not repoint or delete it without fixing that mapping.
9. **`client::zed_urls::terms_of_service` and `ai_privacy_and_security`** exist but describe things Wu does not have. Do not wire new UI to them.
10. **`wu_actions::init()` looks like dead code but is not.** Removing it breaks action registration (see the rustc / rust-ctor issue links in the source comment at `crates/wu_actions/src/lib.rs:6-13`).
11. **`crates/paths::APP_NAME` is the single source of branding for user-data paths.** Changing it silently orphans every existing user's settings, DB, and extensions. `APP_NAME_LOWERCASE` is const-evaluated with assertions (non-empty, ASCII, no path separators, no control characters).
12. **`me.farshed.*` identifiers are duplicated in about eight places** (`crates/wu/Cargo.toml`, `crates/release_channel/src/lib.rs`, `crates/cli/src/main.rs`, `crates/wu/src/main.rs`, `script/install.sh`, `script/uninstall.sh`, `script/bundle-linux`, plus the snap and flatpak templates). Change them together.
13. **Packaging file names still say `zed`** (`crates/wu/resources/windows/zed.iss`, `zed.sh`, `zed.desktop.in`, `zed.entitlements`, `flatpak/zed.metainfo.xml.in`). Build scripts reference them by those names, so renaming means touching `script/bundle-*` too.
14. **Two release channels only** (`dev`, `stable`). Code matching on `ReleaseChannel` is exhaustive over two variants; re-adding Preview/Nightly means auditing `app_id()`, `display_name()`, `dev_name()`, `poll_for_updates()`, `app_identifier()`, `github_asset_name()`, and `.github/workflows/release.yml`.
15. **Do not run `cargo build` casually.** This is a full Zed-sized workspace (~170 crates). Use `./script/clippy` for checks. Release profile pins `codegen-units = 1` for `gpui`, `editor`, `language`, `rope`, `sum_tree`, `text`; `release-fast` is the fast iteration profile.
16. **The repo's own editor config** (`.wu/settings.json`, `.wu/tasks.json`) reveals conventions: prettier for md/json/jsonc/yaml/js/css, 2-space tabs in those, `hard_tabs: false`, `formatter: "auto"`, trailing-whitespace trim on save, final newline on save, `read_only_files` covering `**/*.lock` and `target/**/*.rs`. Tasks: `clippy` runs `./script/clippy`; a second task runs `cargo run --profile release-fast`.
17. **`crates/wu/src/wu.rs` is ~6,200 lines and `crates/wu/src/main.rs` ~1,800.** `.rules` says to prefer extending existing files over creating many small ones -- but for genuinely new logical components add a module under `crates/wu/src/wu/` (existing: `app_menus.rs`, `open_listener.rs`, `open_url_modal.rs`, `quick_action_bar.rs`, `remote_debug.rs`, `visual_tests.rs`, `mac_only_instance.rs`, `windows_only_instance.rs`, `move_to_applications.rs`).
18. **`crates/wu` has three build/test-only bins and feature flags** worth knowing: `inspector`, `tracy`, `track-project-leak`, `test-support`, `visual-tests` (which gates the `wu_visual_test_runner` bin).
19. **Release gating**: `.github/workflows/release.yml` fires on `v*` tags and hard-fails unless the tag matches `version` in `crates/wu/Cargo.toml`. It writes `stable` into `crates/wu/RELEASE_CHANNEL` before bundling.
20. **Windows-specific quirks** live in `crates/explorer_command_injector` (`get_zed_install_folder`, `get_zed_exe_path`), `crates/etw_tracing`, `crates/windows_resources`, and `crates/wu/src/wu/windows_only_instance.rs`.
