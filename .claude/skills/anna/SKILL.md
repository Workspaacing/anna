...
- **`cargo test` with no args only builds `crates/anna`** (`default-members`). Always `-p <crate>` or `--workspace`.
...
the tag matches `version` in `crates/anna/Cargo.toml`). Local
...
`crates/anna_actions` (old `wu::` names still resolve through `deprecated_aliases`). The Cowork agent is
shown to users as "Anna"; its crate, the `cowork` settings key and the `cowork::` action namespace keep
the internal name. The `wu`, `wu_actions` and `wu_env_vars` crates and the `wu` module became `anna`,
`anna_actions`, `anna_env_vars` and `anna` in 1.0.2. The remote server installs into `~/.anna_server` and
clears out what earlier releases left in `~/.wu_server`.

**There are zero `WU_*` env vars** — everything stayed `ZED_*` (`ZED_STATELESS`, `ZED_FILE`,
`ZED_WORKTREE_ROOT`, `ZED_SERVER_URL`, `ZED_LOG`, `ZED_COMMIT_SHA`, ...). `crates/anna_env_vars` holds
...
value still `"Zed"` (UI shows "Wu (Default)"), a `--zed` CLI flag, and `debug.plist` (an orphan; macOS signing uses
`crates/anna/resources/anna.entitlements`).
...
| Need | Crate(s) |
|---|---|
| App entry, boot order, wiring | `crates/anna` (`src/main.rs`, `src/anna.rs`) |
...
Every crate exposes one `pub fn init(cx)` called in order from `crates/anna/src/main.rs`. **Order matters** —
...
5. Possibly update the hardcoded `expected_namespaces` list in `crates/anna/src/anna.rs:5330`
...
- `crates/settings_ui` is optional and unenforced — skip it and the setting is JSON-only, silently.
  ...
  `anna_actions::OpenSettingsPage { page: "<title>", target: None }`.
...
`crates/anna/src/main.rs` and `initialize_panels` in `crates/anna/src/anna.rs:640-678`.
...
- **HLSL is only precompiled in release** — a broken shader passes `cargo check` and fails only in a
  release build.
- `VisualTestAppContext`, `VisualTestPlatform`, `anna_visual_test_runner` and headless screenshot capture
...
- Windows-specific code: `crates/explorer_command_injector`, `crates/etw_tracing`,
  `crates/windows_resources`, `crates/anna/src/anna/windows_only_instance.rs`.
