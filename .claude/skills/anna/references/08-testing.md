...
| `VISUAL_TEST_OUTPUT_DIR`, `UPDATE_BASELINES` | `crates/anna/src/anna/visual_tests.rs` | screenshot dir / baseline refresh |
...
The only image-based golden tests are the **visual tests**: `crates/anna/src/anna/visual_tests.rs` (+ `crates/anna/src/visual_test_runner.rs`). They are `#[ignore]`d, macOS-only, need Screen Recording permission and the main thread. Header docs (`:17-33`):
```bash
cargo test -p anna visual_tests -- --ignored --test-threads=1
UPDATE_BASELINES=1 cargo test -p anna visual_tests -- --ignored --test-threads=1
```
...
# --- whole suite ------------------------------------------------------------
cargo test --workspace                            # NOTE: bare `cargo test` only tests crates/anna
cargo nextest run --workspace                     # honours .config/nextest.toml
cargo nextest run -p editor -E 'test(test_random_split_editor)'

# --- ignored / special ------------------------------------------------------
cargo test -p anna visual_tests -- --ignored --test-threads=1       # macOS only
...
- **`Cargo.toml:185` sets `default-members = ["crates/anna"]`.** A bare `cargo test` / `cargo build` only touches `crates/anna`. Always pass `-p <crate>` or `--workspace`.
...
- **`cargo test` alone tests only `crates/anna`** (`default-members`). Use `-p` or `--workspace`.
...
| visual/golden screenshot tests | `crates/anna/src/anna/visual_tests.rs`, `crates/anna/src/visual_test_runner.rs` |
