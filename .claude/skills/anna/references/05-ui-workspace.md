...
| All action definitions | `crates/anna_actions/src/lib.rs`, plus `crates/menu/src/menu.rs` |
| Global wiring (`init()` call order) | `crates/anna/src/main.rs` (lines ~470-815), `crates/anna/src/anna.rs` |
...
see the LSP status button at `crates/anna/src/anna.rs:489-497`.
...
from `crates/anna/src/main.rs:812`). It reads `component::components()` and renders each
...
1. **New crate** `crates/my_panel/` with `Cargo.toml` containing
   ...
   Add it to the workspace `Cargo.toml` members and to `crates/anna/Cargo.toml` dependencies.
2. **Define the toggle action**: `actions!(my_panel, [ToggleFocus, Toggle]);` in your crate
   (or add to `crates/anna_actions/src/lib.rs` for an `anna::`-namespaced one).
...
6. **Wire it up:** add `my_panel::init(cx);` in `crates/anna/src/main.rs` (near
   `outline_panel::init(cx);`, ~line 635), and inside `initialize_panels` in
   `crates/anna/src/anna.rs:640-678` add `let my_panel = MyPanel::load(workspace_handle.clone(),
...
3. An action (in `anna_actions` or a local `actions!(..)`) plus
...
5. `pub fn init(cx: &mut App) { cx.observe_new(|ws: &mut Workspace, window, cx| { ..
   MyView::register(..) }).detach(); }` and call it from `crates/anna/src/main.rs`.
...
3. ```rust
   ...
   ```
4. Wire in `crates/anna/src/anna.rs`: construct it around lines 470-502 and add
   `status_bar.add_right_item(my_indicator, window, cx);` inside the
   `workspace.status_bar().update(..)` block at `crates/anna/src/anna.rs:503-516`.
...
1. **Define the action.** Either
   ...
   in `crates/anna_actions/src/lib.rs` for `anna::`-namespaced ones. For actions carrying data use
...
let toast = StatusToast::new("Pushed to origin/main", cx, |this, _cx| {
    this.icon(Icon::new(IconName::GitBranch).size(IconSize::Small).color(Color::Muted))
        .action("Create Pull Request", |window, cx| {
            window.dispatch_action(Box::new(anna_actions::git::CreatePullRequest), cx)
        })
});
...
`crates/git_ui_core/src/notifications.rs:53`, `crates/anna/src/anna.rs:625` (`window.prompt`),
...
- Action: `anna_actions::dev::ToggleInspector`. Default keybindings:
...
- Every crate exposes `pub fn init(cx: &mut App)`, called from `crates/anna/src/main.rs`.
  Crates that contain only actions/types also expose an empty `pub fn init()` purely to defeat
  link-time elision (`crates/anna_actions/src/lib.rs:13`, `crates/menu/src/menu.rs:10` explain why).
...
19. **`component::init()` is already called by `workspace::init` (`workspace.rs:843`).** Conversely,
    ...
    `inventory` + link-elision problem that the empty `anna_actions::init()` / `menu::init()` exist to solve.
