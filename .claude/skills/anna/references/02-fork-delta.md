...
App version: `crates/anna/Cargo.toml` = `1.0.0`. Release channel file: `crates/anna/RELEASE_CHANNEL` = `dev`.
...
| **Telemetry, crash reporting, hang reporting** | No `crates/telemetry*`, no `crates/feedback`. `crates/anna/src/reliability.rs` was gutted to *local logging only* (memory + worktree diagnostics via `log::info!`); it uploads nothing. |
...
- Bundle identifiers: **`com.workspaacing.Anna`** (stable) and **`com.workspaacing.Anna-Dev`** (dev):
  - `crates/anna/Cargo.toml:232,240` (`[package.metadata.bundle-dev]` / `bundle-stable`)
  ...
  - One-off: `crates/anna/src/main.rs:154` uses `me.farshed.Oops` for the Linux launch-failure notification.
- Windows AppUserModelID (installer): `Workspaacing.Anna` / `Workspaacing.Anna.Dev`; installer publisher / `CompanyName` is `Workspaacing`.
- Display names: `ReleaseChannel::display_name()` = `"Anna"` / `"Anna Dev"`. The macOS root menu carries the product name (`crates/anna/src/anna/app_menus.rs:60`).
- HTTP User-Agent: `format!("Wu/{} ({}; {})", ...)` -- `crates/anna/src/main.rs:485-490`.
- Binaries: `anna` (`crates/anna/Cargo.toml`, `[[bin]] name = "anna"`, `default-run = "anna"`, package `anna`), `anna_visual_test_runner`. The CLI crate builds a bin named `cli` but presents itself as `anna` (`crates/cli/src/main.rs:51`).
...
| Remote server dir on host | `.anna_server` / `.anna_wsl_server`; the remote server removes old binaries from the legacy `.wu_server` and deletes `.wu_wsl_server` | `paths.rs:83-113`, `cleanup_legacy_server_dir` in `crates/remote_server/src/server.rs` |
| URL schemes | **`anna://`** and **`anna-cli://`**; only `anna://` is registered (old `wu://`, `wu-cli://` and `wu://schemas/...` links are still accepted) | `crates/anna/src/anna/open_listener.rs:134-167`, `crates/client/src/client.rs` (`normalize_legacy_url_scheme`), `crates/cli/src/main.rs:34`, `osx_url_schemes = ["anna"]` in `crates/anna/Cargo.toml` |
...
### Action namespaces
- `crates/anna_actions/src/lib.rs` is the app-level action crate. App actions use `#[action(namespace = anna)]`, giving user-visible IDs like `anna::OpenSettings`, `anna::About`, `anna::OpenDocs`, `anna::Extensions`, `anna::IncreaseBufferFontSize`.
...
- `anna_actions::init()` is an intentionally empty fn that exists only so the crate is not optimized away and its ctor-registered actions run. Called from `crates/anna/src/main.rs:473`. **If you add another actions-only crate, do the same.**
...
**Rule: do not rename these on a whim.** The task variables are user-visible contract (documented in Zed's docs, which Anna's docs explicitly defer to), and the build vars are consumed by `crates/anna/build.rs:46-78` and by CI. A rename would have to be repo-wide, alias-preserving, and deliberate. Do not invent `WU_*` unilaterally -- ask first.

The crate that would be the natural home for env vars, `crates/anna_env_vars/src/anna_env_vars.rs`, currently contains exactly one item and it is `ZED_STATELESS` (with a doc comment that still says "Zed"). It re-exports `env_var::{EnvVar, bool_env_var, env_var}` from `crates/env_var/`.
...
4. **Packaging file names** were `zed.*` until 1.0.2 and are now `crates/anna/resources/windows/anna.iss`, `crates/anna/resources/windows/anna.sh`, `crates/anna/resources/anna.desktop.in`, `crates/anna/resources/anna.entitlements`, `crates/anna/resources/flatpak/anna.metainfo.xml.in`.
...
- `.anna/settings.json` -- excludes `crates/agent/src/tools/evals/fixtures`, a path that no longer exists.
...
- `crates/anna/src/main.rs:138,161` -- the "failed to open a window" message links to `https://zed.dev/docs/linux`.
...
| `crates/anna_env_vars` | Nearly empty | One const (`ZED_STATELESS`). Extend here for new process-level env flags. |
...
2. **Extension registry -> `api.zed.dev`.** `crates/extension_host/src/extension_host.rs:787,951,996` call `http_client.build_zed_api_url(...)`, and `crates/http_client/src/http_client.rs:276-289` maps base `https://zed.dev` to `https://api.zed.dev`. Endpoints: `/extensions...`, `/extensions/{id}/download`, `/extensions/{id}/{version}/download`. This is **deliberate** -- `docs/docs/index.html` states that extensions come from the Zed extension registry and that every Zed extension works.
   - The base comes from the `server_url` setting, default `https://zed.dev` (`assets/settings/default.json:2248`), overridable by `ZED_SERVER_URL` (`crates/client/src/client.rs:32,51`). `crates/anna/src/main.rs:695-698` re-syncs `http.set_base_url(...)` whenever settings change.
...
5. **Doc links** -- `crates/anna/src/anna.rs:101` `DOCS_URL = "https://wu.farshed.me/docs"`, but `crates/release_channel/src/lib.rs:10` `ZED_DOCS_URL = "https://zed.dev/docs"` still backs `ReleaseChannel::docs_url(slug)` for deep links into Zed's reference docs (intentional; Anna's docs defer to Zed's).
...
Including `anna`, `anna_actions`, `anna_env_vars`, `editor`, `workspace`, `project`, `paths`, `client`, `rpc`, `proto`, `remote*`, `auto_update*`, `settings*`, `theme*`, `ui*`, `terminal*`, `git*`, `language*`, `zlog*`.
...
- License attribution for the About dialog is generated by `script/generate-licenses` into `assets/licenses.md`, opened by `anna_actions::OpenLicenses` (`crates/anna/src/anna.rs:196-206`). Config lives in `script/licenses/zed-licenses.toml`.

---

## 6. App boot sequence and where to hook new features in

Entry point: **`crates/anna/src/main.rs`** (`[[bin]] name = "anna", path = "src/main.rs"` in package `anna`; `default-members = ["crates/anna"]` in the root `Cargo.toml`).
...
9. `OpenListener::new()`, then the per-platform single-instance check (skipped when `*anna_env_vars::ZED_STATELESS` or `ReleaseChannel::Dev`).
...
```
trusted_worktrees::init -> menu::init -> anna_actions::init
release_channel::init -> gpui_tokio::init -> AppCommitSha::set_global
settings::init -> zlog_settings::init -> anna::watch_settings_files -> handle_keymap_file_changes
...
Client::set_global
anna::init(cx)                                <- app-level actions live here
[macos] anna::move_to_applications::init
...
theme_extension::init -> command_palette::init -> anna::remote_debug::init -> snippet_provider::init
...
- **New global action or app-level command** -> declare in `crates/anna_actions/src/lib.rs` (`#[action(namespace = anna)]` or `actions!(anna, [...])`), then handle it in `anna::init` (`crates/anna/src/anna.rs:171-286`) for app scope, or in `register_actions` (`crates/anna/src/anna.rs:680`) for workspace scope.
- **New crate with UI** -> add to `[workspace] members` **and** `[workspace.dependencies]` in the root `Cargo.toml`, add it to `crates/anna/Cargo.toml` `[dependencies]`, then call its `init(cx)` in the block above. Order matters: anything that reads settings must come after `settings::init`; anything that needs `AppState` after `AppState::set_global`; anything registering panels before `initialize_workspace`.
- **New panel** -> implement against `crates/workspace/src/dock.rs`, register in `initialize_panels` (`crates/anna/src/anna.rs:641`), and add its `panel_key()` to `PREFERRED_ORDER` in `crates/workspace/src/activity_bar.rs:19` if it should get an activity-bar slot.
...
- **New menu item** -> `crates/anna/src/anna/app_menus.rs`.
- **New `anna://` URL verb** -> `crates/anna/src/anna/open_listener.rs:142-165`.
- **New CLI flag** -> `Args` in `crates/anna/src/main.rs` and the mirrored parser in `crates/cli/src/main.rs`.
...
5. **`ZED_*` env vars are load-bearing.** `crates/anna/build.rs` emits `ZED_COMMIT_SHA` / `ZED_BUILD_ID`; `.github/workflows/release.yml` writes `crates/anna/RELEASE_CHANNEL`; `crates/release_channel/src/lib.rs` does `include_str!("../../anna/RELEASE_CHANNEL")` (with a `__do_not_set_zed_release_channel` cfg escape hatch for nix/crane vendoring). Renaming any of these breaks CI or the build.
...
9. **`anna_actions::init()` looks like dead code but is not.** Removing it breaks action registration (see the rustc / rust-ctor issue links in the source comment at `crates/anna_actions/src/lib.rs:6-13`).
...
11. **`com.workspaacing.*` identifiers (formerly `me.farshed.*`) are duplicated in about eight places** (`crates/anna/Cargo.toml`, `crates/release_channel/src/lib.rs`, `crates/cli/src/main.rs`, `crates/anna/src/main.rs`, `script/install.sh`, `script/uninstall.sh`, `script/bundle-linux`, plus the snap and flatpak templates). Change them together.
12. **Packaging files are referenced by name** (`crates/anna/resources/windows/anna.iss`, `anna.sh`, `anna.desktop.in`, `anna.entitlements`, `flatpak/anna.metainfo.xml.in`) from `script/bundle-*`, the flatpak manifest template and `.gitattributes`, so a rename has to touch all of them.
...
15. **The repo's own editor config** (`.anna/settings.json`, `.anna/tasks.json`) reveals conventions: prettier for md/json/jsonc/yaml/js/css, 2-space tabs in those, `hard_tabs: false`, `formatter: "auto"`, trailing-whitespace trim on save, final newline on save, `read_only_files` covering `**/*.lock` and `target/**/*.rs`. Tasks: `clippy` runs `./script/clippy`; a second task runs `cargo run --profile release-fast`.
16. **`crates/anna/src/anna.rs` is ~6,200 lines and `crates/anna/src/main.rs` ~1,800.** `.rules` says to prefer extending existing files over creating many small ones -- but for genuinely new logical components add a module under `crates/anna/src/anna/` (existing: `app_menus.rs`, `open_listener.rs`, `open_url_modal.rs`, `quick_action_bar.rs`, `remote_debug.rs`, `visual_tests.rs`, `mac_only_instance.rs`, `windows_only_instance.rs`, `move_to_applications.rs`).
17. **`crates/anna` has three build/test-only bins and feature flags** worth knowing: `inspector`, `tracy`, `track-project-leak`, `test-support`, `visual-tests` (which gates the `anna_visual_test_runner` bin).
18. **Release gating**: `.github/workflows/release.yml` fires on `v*` tags and hard-fails unless the tag matches `version` in `crates/anna/Cargo.toml`. It writes `stable` into `crates/anna/RELEASE_CHANNEL` before bundling.
19. **Windows-specific quirks** live in `crates/explorer_command_injector` (`get_zed_install_folder`, `get_zed_exe_path`), `crates/etw_tracing`, `crates/windows_resources`, and `crates/anna/src/anna/windows_only_instance.rs`.
