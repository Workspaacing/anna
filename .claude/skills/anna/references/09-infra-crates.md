...
| `anna_env_vars` | Anna-wide env flags | `ZED_STATELESS` (forces in-memory DB) |
...
| `install_cli` | CLI symlink install + `anna://` URL scheme registration | `InstallCliBinary`, `install_cli_binary`, `RegisterAnnaScheme`, `register_anna_scheme` |
...
```
Set in `crates/anna/src/main.rs` right after `cx.set_http_client(...)`.
...
`remote_server_dir_relative()` = `.anna_server`, `remote_wsl_server_dir_relative()` = `.anna_wsl_server`;
`legacy_remote_server_dir_relative()` = `.wu_server` and `legacy_remote_wsl_server_dir_relative()` = `.wu_wsl_server`
exist only so the remote server can clear them out.
...
Set once in `crates/anna/src/main.rs` (~line 492):
...
are `crates/reqwest_client/src/reqwest_client.rs`, `crates/anna/src/main.rs`,
...
`ZED_STATELESS=1` (`anna_env_vars::ZED_STATELESS`) forces the in-memory fallback DB.
...
`crates/anna/src/main.rs:307`), so **plain `log::info!` / `warn!` / `error!` / `debug!` / `trace!`
...
`crates/anna/RELEASE_CHANNEL` at compile time (`include_str!`), overridable via `ZED_RELEASE_CHANNEL`
...
`crates/anna_env_vars/src/anna_env_vars.rs` (currently just `ZED_STATELESS`).
...
- `ProxySettings { proxy: Option<String> }` with `proxy_url() -> Option<Url>` (falls back to
  ...
  feeds `ReqwestClient::proxy_and_user_agent` in `crates/anna/src/main.rs`.
...
- `client::zed_urls` (doc links), `client::os_info`, `client::APP_URL_SCHEME` (consumed by
  `install_cli::register_anna_scheme`).
...
- `remote_server` — the headless binary deployed to the host under
  `paths::remote_server_dir_relative()` (`.anna_server`; old binaries in the legacy `.wu_server` are removed at startup), with
...
when needed; `CANT_INSTALL_DOCS_URL` for the failure toast) and `RegisterAnnaScheme` /
`register_anna_scheme(cx)`, which calls `cx.register_url_scheme(client::APP_URL_SCHEME)` so `anna://`
...
(`app-icon.ico` for `stable`, `app-icon-dev.ico` otherwise, from `crates/anna/resources/windows`),
