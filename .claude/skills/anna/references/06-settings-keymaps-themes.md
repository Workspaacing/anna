...
| `assets/images/` | 4 | `anna_icon.png` (raster), `anna_logo.svg`, `screenshot-dark.png`, `screenshot-light.png` |
...
binary** — hence the `pub fn init() {}` no-op trick in `anna_actions` and `menu`.
...
`crates/anna/src/anna.rs:1731-1738` (`init_reduce_motion`).
...
`paths::settings_file()` and `paths::global_settings_file()`. Wired up in `crates/anna/src/anna.rs:1740`.
...
(`crates/anna/src/anna.rs:1722-1729`.) For a nested section the idiom is
...
linked (add a `pub fn init() {}` and call it from `anna` if the crate would otherwise be dead-stripped).
...
```
  ...
  (`crates/anna_actions/src/lib.rs:16-21`.)
- `#[action(...)]` options (`action.rs:71-88`): `namespace = x`, `name = "X"`, `no_json`,
  `no_register`, `deprecated_aliases = ["old::Name"]`, `deprecated = "message"`.
- Deprecated aliases in practice: `crates/anna_actions/src/lib.rs:40-62`
  (`anna_actions::OpenSettingsEditor` -> `anna::OpenSettings`, and similar; every former `wu::` name is
...
Where actions live: `crates/anna_actions/src/lib.rs` (736 lines: the `anna::` namespace plus submodule
namespaces such as `anna_actions::settings_profile_selector::Toggle`), `crates/menu/src/menu.rs`
...
`crates/anna/src/anna.rs:1930-1952` (`load_default_keymap`):
...
then the user `keymap.json` on top (`handle_keymap_file_changes`, `anna.rs:1749`).
...
2. **If the namespace is brand new**, add it to the sorted `expected_namespaces` list in
   `crates/anna/src/anna.rs:5330-5444` (`test_action_namespaces`). That test also asserts **every** action
...
2. `crates/anna/src/anna.rs` — `expected_namespaces` (new namespace only)
...
- **User**: `crates/anna/src/main.rs:1621-1662` — reads every file in `paths::themes_dir()`
...
5. `crates/anna/src/anna.rs:5449` (`test_bundled_settings_and_themes`) loads every bundled theme and asserts
...
- The repo own tasks: `C:/Users/USER/Documents/wu-main/.anna/tasks.json`
...
- Picker: `crates/settings_profile_selector/src/settings_profile_selector.rs`, action
  `anna_actions::settings_profile_selector::Toggle` (`:11`). Names come from
...
5. **Dead-code stripping eats action and setting registrations.** If nothing in the binary references a
   crate, its `inventory::submit!` calls never run. That is why `crates/anna_actions/src/lib.rs:13` and
...
13. **Adding an action in a new namespace fails `test_action_namespaces`**
    (`crates/anna/src/anna.rs:5330`). It also fails on any action declared without a namespace.
...
17. **`assets/*.json` are JSONC** — comments and trailing commas everywhere. `.gitattributes` marks
    `*.json` as `linguist-language=JSON-with-Comments`; the repo `.anna/settings.json` maps
...
| An action (`actions!` or `#[derive(Action)]`) | Rebuild. `cargo test -p anna test_action_namespaces` if the namespace is new. Keymap and action schemas regenerate at runtime. |
| Any file in `assets/keymaps/**` | `script/check-keymaps` (bash, needs git). Rebuild. |
| A theme JSON in `assets/themes/**` | Rebuild. `cargo test -p anna test_bundled_settings_and_themes`. |
...
crates/anna_actions/src/lib.rs                       # anna:: namespace actions
...
.anna/settings.json, .anna/tasks.json              # the repo own project config
```

### App init order (`crates/anna/src/main.rs`)

```
settings::init(cx)                                  # main.rs:480
zlog_settings::init(cx)                             # :481
anna::watch_settings_files(fs, cx)                  # :482
