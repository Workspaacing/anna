...
Project: **Anna** (renamed from Wu), built on Zed (`crates/anna` takes the place of Zed's `crates/zed`).
Current version: `crates/anna/Cargo.toml` -> `version = "1.0.0"`.
Release channel in tree: `crates/anna/RELEASE_CHANNEL` -> `dev`.
...
| Build the app (dev) | `cargo build` (default member is `crates/anna`) |
...
cargo build                              # dev profile; default-members = ["crates/anna"]
cargo build --profile release-fast       # release codegen + full debuginfo, lto off
cargo run --profile release-fast         # how the project itself runs the editor (.anna/tasks.json)
...
### Dependency graph
`script/crate-dep-graph` runs `cargo depgraph --workspace-only --offline --root=zed,cli,collab ... | dot -Tsvg > target/crate-graph.html`. STALE - `zed` and `collab` do not exist here. Fix roots to `anna,cli` if you need it.
...
- Channel from `crates/anna/RELEASE_CHANNEL`; version from `crates/anna/Cargo.toml` unless `$env:RELEASE_VERSION` is set.
- Staging dir `<repo>/inno/<arch>` (gitignored `/inno`).
- Builds `anna`, `cli`, `auto_update_helper`, then `explorer_command_injector` (with `--features stable --no-default-features` on the stable channel), then `remote_server`, all `--release --target <arch>-pc-windows-msvc`.
...
- App identity strings must stay in sync with `crates/release_channel/src/lib.rs` (`app_identifier()`) and `crates/anna/src/anna/windows_only_instance.rs` (mutex `Anna-Editor-Stable-Instance-Mutex`).
...
- `default-members = ["crates/anna"]` - a bare `cargo build`/`cargo run` targets the editor only.
...
3. NEVER write a bare `foo = "1.2"` in a member crate. `cargo xtask package-conformity` prints `<dep> is being used as a non-workspace dependency: <crates>` for every violation.
   ...
   - Known pre-existing violations you will see: `crates/anna/Cargo.toml` has `spin = "0.10.0"` and `mimalloc = { version = "0.1", optional = true }` inline. Do not add more.
...
- "prefer specifying the library root path in `Cargo.toml` using `[lib] path = "...rs"` instead of the default `lib.rs`" (e.g. `gpui.rs`, `anna.rs`).
...
```
seen in `crates/anna/build.rs` and every `tooling/xtask/src/tasks/*.rs`.
...
1. `check_version` (ubuntu-24.04) - THE ONLY AUTOMATED CORRECTNESS GATE IN THE REPO:
   ```sh
   version=$(sed -n 's/^version = "\(.*\)"/\1/p' crates/anna/Cargo.toml | head -1)
   [ "v${version}" = "${GITHUB_REF_NAME}" ] || exit 1
   ```
   The git tag must exactly equal `v` + the `version` in `crates/anna/Cargo.toml`. Bump `crates/anna/Cargo.toml` before tagging.
2. `bundle_mac` (macos-14, timeout 300 min): `rustup show`, `brew install lld`, `echo stable > crates/anna/RELEASE_CHANNEL`, optional provisioning profile from `secrets.MACOS_PROVISIONING_PROFILE`, then `script/bundle-mac aarch64-apple-darwin`. Uploads `Anna-aarch64.dmg` + `anna-remote-server-macos-aarch64.gz`, and separately `Anna.dSYM.zip`. `if-no-files-found: error`.
3. `bundle_linux` (matrix: ubuntu-22.04 x86_64, ubuntu-22.04-arm aarch64; 300 min; `CC=clang CXX=clang++`): `./script/linux`, `rustup show`, `echo stable > crates/anna/RELEASE_CHANNEL`, `./script/bundle-linux`.
4. `bundle_windows` (windows-2022, 300 min): `rustup show`, `Set-Content -Path crates/anna/RELEASE_CHANNEL -Value stable -NoNewline`, then `script/bundle-windows.ps1 -Architecture x86_64` under `pwsh`.
...
All three bundle jobs OVERWRITE `crates/anna/RELEASE_CHANNEL` to `stable` at build time; the file is `dev` in the tree.
...
1. THIS WORKING COPY IS NOT A GIT REPOSITORY. `C:/Users/USER/Documents/wu-main/.git` does not exist; `git rev-parse --is-inside-work-tree` fails. Consequences:
   ...
   - `crates/anna/build.rs` degrades gracefully: it tries `git rev-parse HEAD` and, on failure, leaves the commit SHA unset. You can inject `ZED_COMMIT_SHA=<sha>` instead. The build still works.
...
12. `--all-features` in `script/clippy` enables `anna`'s `visual-tests`, `inspector`, `tracy`, `track-project-leak`, and `test-support` features, which pull in `gpui/test-support`, `gpui/leak-detection`, `ztracing/tracy`, `dep:image`, `dep:tempfile`, etc. A change that is fine for the default build can break under the feature union.
...
14. `Cargo.lock` IS COMMITTED (339 KB) and `.anna/settings.json` marks `**/*.lock` read-only in the editor. Do not hand-edit; let cargo regenerate it, and prefer `cargo update -p <one-crate>` over a blanket `cargo update`.
15. `.gitignore` hides generated artifacts you might otherwise think are missing: `/assets/*licenses.*`, `/crates/theme/schemas/theme.json`, `/inno`, `/crates/anna/test_fixtures/visual_tests/`, `.perf-runs`, `.webrtc-sys/`, `**/target`, `**/cargo-target`, `.claude/settings.local.json`, `/node_modules/`, `/script/node_modules`.
16. `.gitattributes` forces `crates/anna/resources/windows/anna.sh` to LF (`text eol=lf`) - it becomes `bin/anna` inside the Windows bundle and must not get CRLF. It also marks `*.json linguist-language=JSON-with-Comments`.
17. TRAILING COMMAS AND COMMENTS IN THIS REPO'S JSON ARE INTENTIONAL (it is JSONC - see `.anna/tasks.json` and `.anna/settings.json`). Do not "fix" them, and do not run stock `prettier --parser=json` over `assets/**`; `script/prettier` passes `--parser=jsonc` for `assets/settings/default.json` specifically.
...
| `C:/Users/USER/Documents/wu-main/.anna/tasks.json` | the two tasks the project itself defines: `./script/clippy`, `cargo run --profile release-fast` |
