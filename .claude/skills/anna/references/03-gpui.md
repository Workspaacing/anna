...
* `crates/anna` features: `inspector` (pulls `gpui/inspector`), `track-project-leak` (pulls
  `gpui/leak-detection`), `test-support`, `visual-tests` (macOS-only in practice).
  `anna` depends on `gpui` with `features = ["stacker"]`.

### Windows build specifics

* `crates/gpui/build.rs` embeds `resources/windows/gpui.manifest.xml` via `embed-resource` when
  `windows-manifest` is on. `crates/anna/Cargo.toml:193` force-enables it for the `anna` binary.
...
`crates/anna/src/visual_test_runner.rs` prints "Visual test runner is only supported on macOS"
...
* Deep element trees can overflow the stack; the `stacker` feature (`stacksafe`) guards
  `Div::{request_layout, prepaint, paint}` and `taffy.rs`. The `anna` binary enables it.
...
| `anna_visual_test_runner`, `--features visual-tests` | stub `main` prints "Visual test runner is only supported on macOS" and exits 1 (`crates/anna/src/visual_test_runner.rs:38-42`) |
